...
const VisitorLog = () => {
    ...
    // const [currentPage, setCurrentPage] = useState(1); // Replaced by 'page'
    // const [itemsPerPage, setItemsPerPage] = useState(10); // Replaced by 'perPage'
    ...
    const loadLogs = React.useCallback(async () => {
        ...
        try {
            const params = {
                page,
                per_page: perPage,
                after: dateRange?.start ? new Date(dateRange.start).toISOString() : undefined,
                before: dateRange?.end ? new Date(dateRange.end).toISOString() : undefined
            };

            const response = await fetchVisitorLog(settings, params);
            // API now returns { data: [], total: N, totalPages: M }
    ...
    }, [settings, page, perPage, dateRange]);
    ...
    // const totalItems = filteredVisits.length; // Now comes from server
    // Total pages is in state.

    // Client-side filtering still happens for "Compare" logic separation, 
    // but effectively paginatedVisits is just filteredVisits because we only fetched one page.
    const paginatedVisits = filteredVisits;

    // Reset page if filters change -- handled by loadLogs dependency on dateRange
    // useEffect(() => {
    //     setCurrentPage(1);
    // }, [filteredVisits]);
